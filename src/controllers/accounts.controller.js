...
export const offlineUsersLastConection= async(req, res) => {
  try {
     ...
     const promise= users.map(async(item)=>{ //si hago forEach revienta (averiguar)
        const last_conection= dayjs(item.last_conection);
        const diff= currentDate.diff(last_conection, "day");
      // console.log(diff)

        if(item.role == "user" && item.status == "active" && diff > 365){
      ...
      .catch((error) => {
        return res.sendRequestError(error)
      });
  } catch (error) {
    res.sendServerError(error.message);
  };
...
export const offlineCollectorLastConection = async(req,res) =>{
  try {
      ...
      let collectorOffline= 0;

      const promise= collectors.map(async(item)=>{
        const last_conection= dayjs(item.last_conection); 
        const diff= currentDate.diff(last_conection, 'month');
        // console.log(diff)
        if( item.status == "active" && diff > 12){
          await CollectorService.update(item._id, {status: "inactive"});
          collectorOffline = collectorOffline + 1;
        };
      });
      ...
      Promise.all(promise)
      .then(()=>{
        res.sendSuccess(`Se han inactivado un total de ${collectorOffline} cuentas`);
      })
      .catch((error) => {
        return res.sendRequestError(error)
      });
